...
import FormModal from './components/NewGoodsModal';
import DictTable from '../../../lib/Components/DictTable';
import NewGoodsActions from './action/NewGoodsActions';
import NewGoodsStore from './store/NewGoodsStore';
import Filter from './components/Filter';
import FormDef from './components/NewGoodsForm';
import CommonAnimate from '@/public/script/CommonAnimate'
const tableName = 'XxxTable';

class NewGoodsPage extends React.Component {
    constructor(props){
        super(props);
        this.state={
            tableData: {
            ...
            pageType:'add',//页面类型，新增（''）的话显示table 其他显示表格页面
            editData:{},//编辑时数据

        }
    }
    componentDidMount(){
        this.unsubcribe = NewGoodsStore.listen(this.onServiceChange);
    ...
    onServiceChange = (data) => {
        ...
        //     let NewGoodsSet = {recordSet,errMsg:'',startPage:data.recordSet.startPage,pageRow:data.recordSet.pageRow,totalRow:data.recordSet.totalRow};
        //     this.setState({NewGoodsSet});
        // }
    }
    ...
    //     // }
    //     // this.setState({loading:true});
    //     // NewGoodsActions.retrieve(filter);
    // }
    // onSearch= ()=> {
    //     this.handleQueryClick();
    // }
    ...
    //     this.state.NewGoodsSet.startPage = current;
    //     this.state.NewGoodsSet.pageRow = pageRow;
    //     this.handleQueryClick();
    // }
    ...
    render() {
      ...
      //  if(pageType){//进入表单
           let  showPage = formPage,
            showTitle = (
                <div className='component-title'>
                    <Divider type="vertical" className='line' />
                    <span className='text'>发布新商品</span>
